...
static void ndt_odom_callback(sensor_msgs::PointCloud2 input)
{
  ...
  gpu::GNormalDistributionsTransform ndt_gpu_inst; //ndt_gpu instance


  // ndt_omp->setNumThreads(n);
  // ndt_omp->setNeighborhoodSearchMethod(pclomp::DIRECT7);

  ndt_gpu_inst.setTransformationEpsilon (epsilon);
  ndt_gpu_inst.setMaximumIterations (iter);
  ndt_gpu_inst.setResolution (res);
  ndt_gpu_inst.setStepSize (step);

  ndt_gpu_inst.setInputSource (tmp);
  ndt_gpu_inst.setInputTarget (filtered_map);

  //std::cout<<"Before aligning"<<std::endl<<*tmp_aligned<<std::endl;
  ndt_gpu_inst.align (ed_guess);
  //std::cout<<"After aligning"<<std::endl<<*tmp_aligned<<std::endl;

  score = ndt_gpu_inst.getFitnessScore();

  if(score < scoreThreshold){

  std::cout << "Normal Distributions Transform has converged after " << ndt_gpu_inst.getFinalNumIteration() << " iters" << std::endl;
  std::cout << "The score is " << score << "\n" << std::endl;

  Eigen::Matrix4f Transf = Eigen::Matrix4f::Identity();
  Transf =  ndt_gpu_inst.getFinalTransformation();
  prevTransf = Transf;
  ...
  pcl::transformPointCloud (*tmp, *tmp_aligned, Transf);
